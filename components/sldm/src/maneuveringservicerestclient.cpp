...
nlohmann::json ManeuveringServiceRestClient::make_SLDM_json(int eventID)
{
    ...
    sldm_json["generation_tstamp"] = now_us;
    sldm_json["eventID"] = eventID;
    sldm_json["event"] = "CLC";
    sldm_json["reference_vehicle_ID"] = m_refVehStationID;

    if (m_db->rangeSelect(m_range_m, m_refVehStationID, returnedvehs) != ldmmap::LDMMap::LDMMAP_OK)
    {
        sldm_json["error"] = "ERROR";
        return sldm_json;
    }
    else
    {
    ...
    if (m_db->lookup(m_refVehStationID, refveh) != ldmmap::LDMMap::LDMMAP_OK)
    {
        sldm_json["error"] = "ERROR_REFERENCE_LOOKUP";
        return sldm_json;
    }
    else
    {
    ...
    for (ldmmap::LDMMap::returnedVehicleData_t vehdata : returnedvehs)
    {
        ...
        auto lVehData = make_vehicle(vehdata.vehData.stationID,
                                     vehdata.vehData.lat,
                                     vehdata.vehData.lon,
...
nlohmann::json  ManeuveringServiceRestClient::make_vehicle(uint64_t stationID,
                                                            double lat,
                                                            double lon,
                                                            ...
                                                            double heading)
{
    ...
    vehicle["lat"] = lat;
    vehicle["lon"] = lon;
    vehicle["speed_ms"] = speed_ms;
    vehicle["turnindicator"] = turnindicator;
    vehicle["CAM_tstamp"] = CAM_tstamp;
    vehicle["GN_tstamp"] = GN_tstamp;
    vehicle["relative_dist_to_reference_m"] = relative_dist_m;
    vehicle["sourceQuadkey"] = src_quadk;
    vehicle["stationType"] = stationType;
    ...
    vehicle["time_since_database_update_us"] = diff_ref_tstamp;
    ...
    {
        vehicle["car_wid_mm"] = car_width_mm.getData();
    }
    else
    {
