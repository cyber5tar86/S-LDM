...
namespace QuadKeys
{
    ...
    std::vector<std::string> QuadKeyTS::unifyQuadkeys2(std::vector<std::string> quadKeys)
    {
        ...
        std::sort(std::begin(quadKeys), std::end(quadKeys), [](const std::string &lhs, const std::string &rhs)
    ...
    void QuadKeyTS::checkdim(std::vector<std::string> &quadKeys)
    {
        ...
        std::cout << "[QUADKEYS] Checking dimensions: " << std::accumulate(vf.begin(), vf.end(), -4, [](int sum, const std::string &elem)
                                                                           { return sum + elem.size() + 21; }) +
                                                               180
                  << std::endl;

        if (std::accumulate(vf.begin(), vf.end(), -4, [](int sum, const std::string &elem)
                            { return sum + elem.size() + 21; }) +
                180 >
            (max_length))
        {
        ...
        while (std::accumulate(vf.begin(), vf.end(), -4, [](int sum, const std::string &elem)
                               { return sum + elem.size() + 21; }) +
                   180 >=
               max_length)
        {
        ...
        std::cout << "[QUADKEYS] Final dimension: " << std::accumulate(vf.begin(), vf.end(), -4, [](int sum, const std::string &elem)
                                                                       { return sum + elem.size() + 21; }) +
                                                           180
                  << std::endl;
