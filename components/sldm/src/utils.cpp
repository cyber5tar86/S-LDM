...
uint64_t
get_timestamp_ms_cam(void)
{
    return (static_cast<uint64_t>(floor(get_timestamp_us() / 1000.0)) - TIME_SHIFT_MILLI) % 65536;
}

int
timer_fd_create(struct pollfd &pollfd, int &clockFd, uint64_t time_us)
{
...
std::string
exteriorLights_bit_to_string(uint8_t extLights)
{
...
bool
doublecomp(double d1, double d2, double eps)
{
    return std::abs(d1 - d2) < eps;
}

int
logfprintf(FILE *stream, std::string modulename, const char *format, ...)
{
    va_list arg;
    int retval;
