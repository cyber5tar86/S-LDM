...
bool indicatorTriggerManager::checkAndTrigger(double lat, double lon, uint64_t refVehStationID, uint8_t exteriorLightsStatus)
{
    ...
    if (exteriorLightsStatus & (1 << 4) || (m_left_indicator_enabled == true && exteriorLightsStatus & (1 << 5)))
    {
        ...
        if (std::find(m_already_triggered.begin(), m_already_triggered.end(), refVehStationID) == m_already_triggered.end())
        {
            ...
            // ManeuveringServiceRestClient *ms_restclient = new(std::nothrow) ManeuveringServiceRestClient(lat,lon,refVehStationID,m_db);

            std::cout << "[INDICATOR TRIGGER MANAGER] Contacting the REST server at: " << options_string_pop(m_opts->ms_rest_addr) << ":" << m_opts->ms_rest_port << std::endl;

            ManeuveringServiceRestClient *ms_restclient =
                new (std::nothrow) ManeuveringServiceRestClient(refVehStationID, m_db, std::string(options_string_pop(m_opts->ms_rest_addr)), m_opts->ms_rest_port);

            if (ms_restclient != nullptr)
            {
                ms_restclient->changeContextRange(m_opts->context_radius);
                ms_restclient->setPeriodicInterval(m_opts->ms_rest_periodicity);
